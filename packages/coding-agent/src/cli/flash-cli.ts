...
const ramWrapperSource = path.join(repoRoot, "scripts/omomp-ram-wrapper.sh");
...
export function buildRamFilter(entries: readonly PortableAgentEntry[] = PORTABLE_AGENT_MANIFEST): string {
	const lines = ["# generated by omomp flash; databases use sqlite3 .backup"];
...
async function copySelected(source: string, destination: string): Promise<void> {
	...
	if (stat.isSymbolicLink()) {
		...
		await fs.cp(source, destination, { recursive: true, preserveTimestamps: true, verbatimSymlinks: true });
	} else {
...
async function createAndMount(
	...
	for (const [index, code] of [[1, "EF02"], [2, "EF00"], [3, "8309"]] as const) {
		if (!(await capture(["sgdisk", "-i", String(index), device.path])).toUpperCase().includes(code)) {
			throw new FlashError(`partitioned postcondition failed: partition ${index} type differs`);
		}
	}
...
async function phaseSystemConfigured(target: MountedTarget): Promise<void> {
	...
	await fs.writeFile(localeGenPath, localeGen.replace(/^#?en_US\.UTF-8 UTF-8$/m, "en_US.UTF-8 UTF-8"));
...
async function phaseRamModeInstalled(target: MountedTarget, bundle: PortableBundle): Promise<void> {
	...
	if (!(await exists(ramWrapperSource))) throw new FlashError(`RAM wrapper is missing from this build: ${ramWrapperSource}`);
	await fs.copyFile(ramWrapperSource, path.join(bin, "omomp"));
...
async function verifyPhase(phase: FlashPhase, target: MountedTarget, device: DeviceInfo, user: PayloadUser, payload: StagedPayload): Promise<void> {
	...
	if (phase === "base-installed") {
		...
		if (!binaryFilename || !nativeFilename) throw new FlashError("ram-mode-installed postcondition failed: bundle manifest lacks filenames");
		if (
		...
		for (const [index, code] of [[1, "EF02"], [2, "EF00"], [3, "8309"]] as const) {
			if (!(await capture(["sgdisk", "-i", String(index), device.path])).toUpperCase().includes(code)) fail(`partition ${index} type differs`);
		}
	} else if (phase === "formatted") {
		if ((await capture(["blkid", "-o", "value", "-s", "TYPE", target.esp])) !== "vfat") fail("ESP is not FAT32");
		if (!(await capture(["cryptsetup", "luksDump", target.luks])).includes("Version:       2")) fail("root is not LUKS2");
