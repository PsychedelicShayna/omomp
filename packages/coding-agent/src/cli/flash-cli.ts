...
export function buildRamFilter(entries: readonly PortableAgentEntry[] = PORTABLE_AGENT_MANIFEST): string {
	const lines = ["# generated by omomp flash; databases use sqlite3 .backup", "- /*.db", "- /*.db-wal", "- /*.db-shm"];
...
async function probeState(device: string): Promise<FlashState | null> {
	...
	try {
		...
		process.stderr.write(
			chalk.yellow(`flash: state probe failed; treating the device as untracked: ${String(error)}\n`),
		);
		return null;
	}
}
...
async function verifyPhase(
	...
	if (phase === "base-installed") {
		...
		if (
			(await fs.readlink(path.join(target.root, "usr/local/libexec/omomp-portable"))) !==
			"../lib/omomp-portable/omomp"
		)
...
async function flash(cmd: FlashCommandArgs): Promise<void> {
	...
	if (cmd.flags.resume && !previousState)
		throw new FlashError("--resume requested, but no readable OMOMP flash state exists");
