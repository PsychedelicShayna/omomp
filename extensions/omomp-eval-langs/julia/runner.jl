# OMP Julia runner — subprocess wrapper used by the extension-provided eval
# backend (see ../kernel.ts for the shared NDJSON protocol this speaks).
#
# Persistent Julia process that reads one JSON request per line from stdin
# ({id, code, cwd?, env?, silent?}) and replies with frames on stdout:
#   {type:"started", id}
...
#
# Each cell is `Core.eval`d into `Main` so top-level bindings, functions, and
# `using` imports survive across cells, mirroring the Python/Ruby runners.
...
# from `show(io, MIME"image/png", plt)` itself.
...
function json_serialize(v)
    ...
    elseif v isa Bool
        return v ? "true" : "false"
    elseif v isa Real
        return isfinite(v) ? (v isa Integer || v isa AbstractFloat ? string(v) : string(Float64(v))) : "null"
    elseif v isa Number
...
function emit_error(rid, err, bt)
    ...
    # self-contained error display, matching the Python and Ruby runners.
...
function apply_request_runtime(cwd, env)
    try
        if cwd !== nothing && !isempty(cwd)
            cd(cwd)
        end
    catch
        # ignore
...
function emit_protocol_error(id, message)
    emit_frame(Dict(
        "type" => "error",
        "id" => id,
        "ename" => "ProtocolError",
...
function main()
    while !eof(ORIGINAL_STDIN)
        ...
        try
            ...
            emit_protocol_error("", "Invalid JSON request: $(err)")
            continue
        end
        ...
        if get(req, "type", nothing) == "exit"
            break
        end

        rid = string(get(req, "id", ""))
        if isempty(rid)
            continue
        end
        silent = get(req, "silent", false) === true
        code = string(get(req, "code", ""))
        cwd = get(req, "cwd", nothing)
        env = get(req, "env", nothing)

        global current_rid = rid
        exec_status = "ok"
        try
            emit_frame(Dict("type" => "started", "id" => rid))
            apply_request_runtime(cwd, env)
            ...
            exec_status = err isa InterruptException ? "cancelled" : "error"
        ...
        emit_frame(Dict(
            ...
            "status" => exec_status,
            "executionCount" => 1,
            "cancelled" => exec_status == "cancelled"
        ))
        global current_rid = nothing
    end
end
